...
struct NowPlayingView: View {
    ...
    /// On a phone-sized screen the lyrics take the cover's place when asked for.
    @State private var showLyrics = false

    var body: some View {
        NavigationStack {
            GeometryReader { geo in
                // Unfolded, or on an iPad, there's room for the player and the lyrics side by
                // side. Width alone isn't enough: a phone on its side is wide but too short.
                let wide = LyricsFeature.embedded && geo.size.width >= 640 && geo.size.height >= 520
                Group {
                    if wide {
                        HStack(spacing: 32) {
                            playerColumn(showHero: true, wide: true)
                        ...
                        playerColumn(showHero: !showLyrics, wide: false)
                    }
                }
                .frame(width: geo.size.width, height: geo.size.height)
            }
            // On an iPad or an unfolded phone a form sheet would leave no room for the lyrics.
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    ...
                    // The bar button would squeeze it to "E…".
                    .fixedSize()
    ...
    private func playerColumn(showHero: Bool, wide: Bool) -> some View {
        VStack(spacing: 0) {
            if showHero {
                Spacer()
            ...
            VStack(spacing: 6) {
                if let track = player.currentTrack, !track.isEmpty {
                    ...
                    if !LyricsFeature.embedded {
                        LyricsLinkButton(title: track, artist: player.currentArtist,
                ...
                // Lyrics or cover, on a phone-sized screen; side by side is always both.
                if LyricsFeature.embedded && !wide {
                    lyricsToggle.padding(.top, 10)
                }
            }
            .padding(.horizontal, 32)
            .padding(.bottom, showHero ? 48 : 24)
    ...
    private var lyricsToggle: some View {
        Button {
            ...
            Label(NSLocalizedString(showLyrics ? "hide_lyrics" : "view_lyrics", comment: ""),
                  systemImage: showLyrics ? "photo" : "quote.bubble")
...
struct NowPlayingBar: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Button { navigation.showNowPlaying = true } label: {
                HStack(spacing: 12) {
                    if let station = player.currentStation {
        ...
        .sheet(isPresented: $navigation.showNowPlaying) {
